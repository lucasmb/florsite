...
(function () {
	...
	fullScroll.prototype.buildPublicFunctions = function () {
		...
		this.mouseWheelAndKey = function (event) {
			var blocked;							
			blocked = document.getElementById('blocked-scroll').getAttribute('data-blocked');
			console.log('blocked wheel: '+ blocked);
			console.log(_self.defaults.currentPosition);
				if (event.deltaY > 0 || event.keyCode == 40) {	
					if(blocked!=='1' || _self.defaults.currentPosition== 0) {
						console.log('IF: ');
						console.log(blocked !== '1');
						console.log(_self.defaults.currentPosition== 0);


						console.log(blocked!==1 || _self.defaults.currentPosition== 0)
						console.log('aumenta');
		...
		this.hashChange = function (event) {
			if (location) {
				var anchor = location.hash.replace('#', '').split('/')[0];
				console.log('song: '+ currentSongNum);
				if( parseInt(anchor) < 15 && parseInt(anchor) > 0){
					console.log('blockeoloco');
					currentSongNum= parseInt(anchor)-1;
					window.blockScroll();
				}
				if(parseInt(anchor) == 0 || parseInt(anchor) == 14)
						window.allowScroll();
				//
				if (anchor !== "") {
					if (anchor < 0) {
		...
		this.changeCurrentPosition = function (position) {
			var blocked;							
			console.log(this.defaults.sections[position]);
			blocked = document.getElementById('blocked-scroll').getAttribute('data-blocked');
			console.log('position: '+ position);
			console.log('blockedCurrent: '+ blocked);

			//added  to stop scrolling
				if (position !== "" && blocked !== '1' ) {
					_self.defaults.currentPosition = position;
					location.hash = _self.defaults.currentPosition;
					document.getElementById('blocked-scroll').setAttribute('data-blocked', 0);
				}
		};
